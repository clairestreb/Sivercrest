    var url = '/client/holdings/getotherassets?isGroup=' + $("#isGroup").val() + '&isClientGroup=' + $("#isClientGroup").val()
                    + '&contactId=' + $("#contactId").val() + '&entityId=' + $("#entityId").val();
    allCommands(url);

    // To put arrow below text when page Loads
    var sortedColumn = $("#assetsTable .k-grid-header tr th[data-field='MarketValueTotal']");
...
function allCommands(url) {
    ...
    $("#assetsTable").kendoGrid({
        sortable: true,
        scrollable: true,
        excel: { fileName: "OtherAssetHoldings.xlsx" },
        ...
        columns: [
            {
                ...
                groupFooterTemplate: "TOTAL OTHER ASSETS",
...
/*                    { field: "Quantity", aggregate: "sum" },
    ...
    var grid = $("#assetsTable").data("kendoGrid");
...
function getExcel() {
    var a = $("#assetsTable").getKendoGrid();
    a.saveAsExcel();
}
