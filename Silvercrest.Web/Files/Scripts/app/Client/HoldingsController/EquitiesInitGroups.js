var url = '/Client/Holdings/GetEquities?isGroup=' + $("#isGroup").val() + '&isClientGroup=' + $("#isClientGroup").val()
...
var groupedSortedColumn = $("#equitiesGroupTable .k-grid-header tr th[data-field='MarketValueTotal']");
...
function allCommands(url) {
	...
	$("#equitiesGroupTable").kendoGrid({
		pdf: {
			allPages: true,
		},
		scrollable: true,
		excel: { fileName: "EquitiesHoldings-Grouped.xlsx" },
					...
					for (var j = 0; j < row.cells.length; j++) {
						...
						if (cell.value == "DATE")
						    for (var ro = i + 1; ro < sheet.rows.length; ro++) {
								...
								sheet.rows[ro].cells[j + 3].value = kendo.toString(kendo.parseDate(sheet.rows[ro].cells[j + 3].value, 'yyyy-MM-dd'), 'MM/dd/yyyy');
						    }
					}
		...
		columns: [
            ...
            {
            	field: "Symbol", title: "SYMBOL", width: 70,
...
*/
		    ...
		    {
	            hidden: true, field: "CategorySort"
            },
            {
                    hidden: true, field: "SubCategorySort"
            }
		...
		//	$("#equitiesGroupTable").find(".k-group-col,.k-group-cell").remove();
		//	var spanCells = $("#equitiesGroupTable").find(".k-grouping-row").children("td");
	...
	var grid = $("#equitiesGroupTable").data("kendoGrid");
	...
	function sortgroups() {
		...
		var h = $("#equitiesGroupTable .k-grouping-row");
		...
		var t = $("#equitiesGroupTable .k-group-footer");
		$.each(t, function (key, elem) {
		    if ($(elem).next().hasClass("groupingOne") || $(elem).is($("#equitiesGroupTable tr:last"))) {
		...
		$("#equitiesGroupTable a.k-link").removeClass("k-link");
	}
...
function onClickSort(e) {
	...
	var target = $(this);
	//remove from another
	$("#equitiesGroupTable th.k-header").children().each(function (index, elem) {
		if ($(elem).is("span")) {
			if (index > 0) {
...
function getExcelGroups() {
	$("#equitiesGroupTable").getKendoGrid().saveAsExcel();
}
